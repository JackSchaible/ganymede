...
export class CrCalculatorComponent implements OnInit {
  ...
  bab = new FormControl("", Validators.required);
  ac = new FormControl("", Validators.required);
  hp = new FormControl("", Validators.required);
  dps = new FormControl("", Validators.required);

  crForm: FormGroup = new FormGroup({
    bab: this.bab,
    ac: this.ac,
    hp: this.hp,
    dps: this.dps
  ...
  ngOnInit() {
    this.crForm.valueChanges.subscribe(x => {
      if (x.ac && x.hp && x.bab && x.dps) this.calculateCr();
    });
  }
  ...
  calculateCr() {
    ...
    hpRating = this.getHPRating(this.hp.value);
    acRating = this.getACRating(this.ac.value);
    dpsRating = this.getDPSRating(this.dps.value);
    babRating = this.getBABRating(this.bab.value);
    ...
    this.cr = Math.ceil((defensiveCR + offensiveCR) / 2);
    this.xp = this.getXP(this.cr);
  }

  getXP(cr: number): number {
    let xp = 0;
  ...
  getHPRating(value: number): number {
    let rating = 0;
  ...
  getACRating(value: number): number {
    let rating = 0;
  ...
  getDPSRating(value: number): number {
    let rating = 0;
  ...
  getBABRating(value: number): number {
    let rating = 0;
